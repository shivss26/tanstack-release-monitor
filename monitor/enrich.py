...
def run_agent(context_md, system, transcript, metrics):
    ...
    tools = list(tanstack_tool.TOOLS) + SERVER_TOOLS
    ...
    for i in range(MAX_CONTINUATIONS):
        ...
        for b in resp.content:
            if b.type == "thinking" and (getattr(b, "thinking", "") or "").strip():
                ...
                transcript.append(f"[tool_use] {b.name}({json.dumps(b.input)})")
        ...
        if resp.stop_reason == "tool_use":
            ...
            for b in resp.content:
                if b.type == "server_tool_use":
                    metrics["web_calls"] += 1
                if b.type == "tool_use" and b.name in tanstack_tool.TOOL_NAMES:
                    metrics["doc_calls"] += 1
                    out, is_err = tanstack_tool.dispatch(b.name, b.input)
                    transcript.append(f"[tool_result {b.name} is_error={is_err}]\n{out[:600]}")
...
def build_summary(meta, bullets, noise_count, detected_ist, stamp, label=""):
    ...
    L = ["# TanStack release summary", "",
         f"- Repository: {repo}",
         f"- Library: {libs}",
         f"- Rollup released: {fmt_ist(released)}",
         f"- Detected by monitor: {fmt_ist(detected_ist)}"]
    if meta.get("html_url"):
        L.append(f"- Release: {meta['html_url']}")
    L.append("")
...
def run(tag, detected_at, samples_dir, out_dir):
    ...
    print(f"  iterations={metrics['iterations']} doc_calls={metrics['doc_calls']} "
          f"web_calls={metrics['web_calls']} format_matched={meta.get('format_matched', True)}")
    return summary
